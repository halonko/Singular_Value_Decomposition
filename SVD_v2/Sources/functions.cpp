...
void SVD(my_matrix &matrix, vector<double> &owner_double, vector<my_matrix> &owner_u, vector<my_matrix> &owner_v,
         size_t k, const double epsilon) {
    size_t n = matrix.column_size();
    size_t m = matrix.row_size();
    if (k == 0) {
        k = min(n, m);
...
void run_SVD_on_matrix(vector<vector<double>> &array, size_t k, const double epsilon) {
    ...
    my_matrix sigma(k, k);
    my_matrix u(k, matrix.column_size());
    my_matrix v(k, matrix.row_size());
    for (size_t i = 0; i < k; ++i) {
